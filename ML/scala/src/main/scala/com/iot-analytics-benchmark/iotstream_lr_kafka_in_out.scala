...
In one window:
  $ scala -cp <path>iotstream_<scala version>-<code version>.jar:/root/kafka/libs/<asterisk> com.iotstream.sim_sensors_lr_kafka n_sensors average_sensor_events_per_second \
...
object iotstream_lr_kafka_in_out {
  def main(args: Array[String]) {
    ...
    props.put("bootstrap.servers", "hpc0982:9092")
    ...
    def run_model(rdd: RDD[String]): Unit = {
      ...
      if (rdd.count == 0) {
        ...
        if (model.predict(Vectors.dense(features)) > 0) {
          println("\033[31m%s: Interval %d: Attention neeeeded (%d sensor events in interval)\033[0m".format(Instant.now.toString, interval.value, input.length))
          sendAsync("\033[31m%s: Interval %d: Attention neeeeded (%d sensor events in interval)\033[0m".format(Instant.now.toString, interval.value, input.length))
        } else {
          ...
          sendAsync("%s: Interval %d: Everything is OK (%d sensor events in interval)".format(Instant.now.toString, interval.value, input.length))

        }
        //        val data = new ProducerRecord[String, String]("res","hpc0982", "%s: Interval %d: Everything is OK (%d sensor events in interval)".format(Instant.now.toString, interval.value, input.length))
        //        kaf_producer.send(data)
